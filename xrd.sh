...
local arr_log=($(filename "${__XRD_LOG}"))
local arr_pid=($(filename "${__XRD_PIDFILE}"))

local XRD_LOG="$(dirname "${__XRD_LOG}")/${arr_log[0]}_xrd.${arr_log[1]}"
local CMSD_LOG="$(dirname "${__XRD_LOG}")/${arr_log[0]}_cmsd.${arr_log[1]}"

local XRD_PID="$(dirname "${__XRD_PIDFILE}")/${arr_pid[0]}_xrd.${arr_pid[1]}"
local CMSD_PID="$(dirname "${__XRD_PIDFILE}")/${arr_pid[0]}_cmsd.${arr_pid[1]}"

local XRD_START="/usr/bin/xrootd -b ${__XRD_DEBUG} -n ${__XRD_INSTANCE_NAME} -l ${XRD_LOG} -s ${XRD_PID} -c ${CFG}"
local CMSD_START="/usr/bin/cmsd -b ${__XRD_DEBUG} -n ${__XRD_INSTANCE_NAME} -l ${CMSD_LOG} -s ${CMSD_PID} -c ${CFG}"
